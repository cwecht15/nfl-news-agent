...
- **Concurrent runs no longer lose their data.** Two workflows that start in the same second slip
  past the shared `daily-pipeline` concurrency group (the local tasks catch up together after the
  PC wakes, and dispatch at once). Both then write `data/reports/<date>.*`, and the loser's push
  is rejected; a plain `git rebase` stopped on the conflict and the run exited 1 with its work
