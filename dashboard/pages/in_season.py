...
tab_week, tab_roster, tab_injury, tab_inactives, tab_audit = st.tabs(
    ["Week", "Roster State", "Injury Report", "Inactives", "Projection Audit"]
)
...
# ─── Inactives ───
with tab_inactives:
    inact_dir = get_data_dir("inactives") / str(ctx.season)
    inact_weeks = sorted(
        (int(p.stem[2:]) for p in inact_dir.glob("wk*.json") if p.stem[2:].isdigit()), reverse=True,
    )
    if not inact_weeks:
        ...
        idata = _load_json(inact_dir / f"wk{iw:02d}.json") or {}
